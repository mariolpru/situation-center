  ...
  if (hs) {
    ...
    hs.innerHTML = POINTS.map(p => `
      <button class="hotspot" style="left:${p.x}%; top:${p.y}%"
        data-open-modal="${p.modal}" aria-label="${p.label}">${GLYPH[p.icon]}</button>`).join("");
  }
})();
